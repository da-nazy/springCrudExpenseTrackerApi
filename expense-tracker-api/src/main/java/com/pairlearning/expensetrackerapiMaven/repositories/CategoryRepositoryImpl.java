...
public class CategoryRepositoryImpl implements CategoryRepository {
    // finds category and transaction with  categoryid
    //COALESCE(SUM(T.AMOUNT),0) TOTAL_EXPENSE get total T for transactions get amount in transactions and sum it return 0 if null.
    ...
    public void removeById(Integer userId, Integer categoryId) {
     this.removeAllCartTransactions(categoryId);
     jdbcTemplate.update(SQL_DELETE_CATEGORY,new Object[]{userId,categoryId});
    }

    private void removeAllCartTransactions(Integer categoryId){
        jdbcTemplate.update(SQL_DELETE_ALL_TRANSACTIONS,new Object[]{categoryId});
    }

    private RowMapper<Category> categoryRowMapper=((rs,row)->{
        return new Category(
